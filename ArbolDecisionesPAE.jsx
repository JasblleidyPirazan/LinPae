...
const CONDITION_MAPPINGS = {
  ...
  'sedes en zonas rurales de difícil acceso': { variable: 'zona', matches: ['Rural de difícil acceso'] },
  'sede educativa = ubicada en zona rural de difícil acceso': { variable: 'zona', matches: ['Rural de difícil acceso'] },
  'sede educativa = zona rural de difícil acceso': { variable: 'zona', matches: ['Rural de difícil acceso'] },
  'sede = zona rural de difícil acceso': { variable: 'zona', matches: ['Rural de difícil acceso'] },
  // --- Modalidad ---
  ...
  'canasta pae en casa': { variable: 'modalidad', matches: ['Canasta PAE en Casa'] },
  'canasta_pae_en_casa': { variable: 'modalidad', matches: ['Canasta PAE en Casa'] },
  'bono alimentario': { variable: 'modalidad', matches: ['Bono Alimentario'] },
  'bono_alimentario': { variable: 'modalidad', matches: ['Bono Alimentario'] },
  // --- Tipo ETC ---
  'etc sea departamental con operación descentralizada': { variable: 'tipoETC', matches: ['Departamental'] },
  'etc departamental con operación descentralizada': { variable: 'tipoETC', matches: ['Departamental'] },
  ...
  'modelo diferencial indígena': { variable: 'modeloAtencion', matches: ['Pueblos indígenas'] },
  'modelo_indigena': { variable: 'modeloAtencion', matches: ['Pueblos indígenas'] },
  'modelo indígena': { variable: 'modeloAtencion', matches: ['Pueblos indígenas'] },
  'territorio_indigena': { variable: 'modeloAtencion', matches: ['Pueblos indígenas'] },
  'territorio indígena': { variable: 'modeloAtencion', matches: ['Pueblos indígenas'] },
  'poblacion_mayoritariamente_indigena': { variable: 'modeloAtencion', matches: ['Pueblos indígenas'] },
  'modelo_NARP': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'modelo_narp': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'modelo NARP': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'territorio_NARP': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'territorio_narp': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'sede_en_territorio_NARP': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'sede_en_territorio_narp': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'matricula_narp > 50%': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'sede_con_pec': { variable: 'modeloAtencion', matches: ['Comunidades NARP'] },
  'residencias_escolares': { variable: 'modeloAtencion', matches: ['Residencias escolares'] },
  'residencias escolares': { variable: 'modeloAtencion', matches: ['Residencias escolares'] },
  // --- Situación ---
  ...
  'fuerza_mayor': { variable: 'situacion', matches: ['Emergencia declarada'] },
  'caso_fortuito': { variable: 'situacion', matches: ['Emergencia declarada'] },
  'grave_perturbacion_orden_publico': { variable: 'situacion', matches: ['Emergencia declarada'] },
  // --- Financiación ---
  ...
  'recursos del PGN': { variable: 'financiacion', matches: ['PGN'] },
  'recursos PGN': { variable: 'financiacion', matches: ['PGN'] },
  // --- Tipo operador ---
  'se vincule organizaciones de base comunitaria como operadores': { variable: 'tipoOperador', matches: ['Organización de base comunitaria'] },
  'organizaciones de base comunitaria como operadores': { variable: 'tipoOperador', matches: ['Organización de base comunitaria'] },
  'organizaciones de base comunitaria': { variable: 'tipoOperador', matches: ['Organización de base comunitaria'] },
};
...
const FILTER_CONFIG = [
  ...
  { key: 'modalidad', label: 'Modalidad', options: [
    'Todas', 'Preparada en sitio', 'Caliente transportada', 'Industrializada',
    'Canasta PAE en Casa', 'Bono Alimentario'
  ]},
  { key: 'tipoETC', label: 'Tipo ETC', options: ['Todos', 'Departamental', 'Municipal certificada'] },
  { key: 'nivelEscolar', label: 'Nivel escolar', options: [
    'Todos', 'Preescolar / 2do ciclo ed. inicial', 'Primaria', 'Secundaria'
  ]},
  { key: 'jornada', label: 'Jornada', options: [
    'Todas', 'Media jornada', 'Completa (6h)', 'Extendida (8h)', 'Jornada única'
  ]},
  { key: 'enfoqueDif', label: 'Enfoque dif.', options: ['Todos', 'Ninguno', 'Indígena', 'NARP'] },
  { key: 'modeloAtencion', label: 'Modelo atención', options: [
    'Todos', 'Convencional', 'Pueblos indígenas', 'Comunidades NARP',
    'Rural difícil acceso', 'Residencias escolares'
  ]},
  { key: 'situacion', label: 'Situación', options: [
    'Todas', 'Operación normal', 'Receso estudiantil', 'Emergencia declarada'
  ]},
  { key: 'financiacion', label: 'Financiación', options: ['Todas', 'SGP', 'SGR', 'PGN', 'Mixta'] },
  { key: 'tipoOperador', label: 'Tipo operador', options: [
    'Todos', 'Operador convencional', 'Organización de base comunitaria', 'FSE / AMPF'
  ]},
];
...
function resolveParentheses(text, filters) {
  ...
  while (result.includes('(') && safety-- > 0) {
    ...
    let resolved;
    if (results.some(r => r === 'match')) resolved = 'match';
    else if (results.every(r => r === 'nomatch')) resolved = 'nomatch';
    else resolved = 'unevaluable';

    // Apply NOT inversion
    if (hasNot) {
      if (resolved === 'match') resolved = 'nomatch';
      else if (resolved === 'nomatch') resolved = 'match';
      // unevaluable stays unevaluable
    }

    // Replace the full match with a placeholder
    const placeholder = resolved === 'match' ? '__MATCH__'
      : resolved === 'nomatch' ? '__NOMATCH__'
      : '__UNEVALUABLE__';
...
function evaluateCondition(condText, filters) {
  ...
  const parts = isAnd
    ? resolved.split(/\s+AND\s+/i)
    : isOr
      ? resolved.split(/\s+(?:OR|O)\s+/i)
      : [resolved];
...
const FILTER_INCOMPATIBILITIES = [
  ...
  },
  {
    ...
    check: f => f.nivelEscolar === 'Preescolar / 2do ciclo ed. inicial'
      && (f.jornada === 'Completa (6h)' || f.jornada === 'Extendida (8h)')
      && f.modalidad === 'Industrializada',
  ...
  },
  {
    ...
    check: f => f.modeloAtencion === 'Residencias escolares'
      && f.modalidad !== 'Todas' && f.modalidad !== 'Preparada en sitio' && f.modalidad !== 'Caliente transportada',
  ...
  },
  {
    ...
    check: f => f.modalidad === 'Canasta PAE en Casa'
      && f.situacion !== 'Todas' && f.situacion !== 'Receso estudiantil' && f.situacion !== 'Emergencia declarada',
  ...
  },
  {
    ...
    check: f => f.modalidad === 'Bono Alimentario'
      && f.situacion !== 'Todas' && f.situacion !== 'Emergencia declarada',
...
function FilterIncompatibilityWarnings({ filters }) {
        ...
        <div key={w.id} style={{
          ...
          <div>
            <div style={{ fontWeight: 700, marginBottom: 2 }}>
              {w.severity === 'error' ? 'Combinación no permitida' : 'Advertencia de compatibilidad'}
            </div>
            <div>{w.message}</div>
            <div style={{ fontSize: 10, marginTop: 4, opacity: 0.7 }}>Ref: {w.ref}</div>
          </div>
        </div>
      ))}
    </div>
...
export default function ArbolDecisionesPAE() {
  ...
  return (
    <div style={{
      ...
      {/* Filter Incompatibility Warnings */}
      <FilterIncompatibilityWarnings filters={filters} />
